...
/**
 * Daily Activity Tracker — Gold/Black Theme (Fixed)
 * - Centers content, big heading
 * - Fixed bottom navigation (safe-area aware)
 * - Line SVG icons
 * - LocalStorage only; CSV export
 *
 * Bugfixes in this revision:
 * - Removed stray "$1" placeholders that caused ReferenceError
 * - Added safe default for Icon `d` prop to avoid undefined path errors
 * - Flattened categories to list options directly (no intermediary "Options")
 * - Added lightweight runtime self-tests to catch regressions early
 * - Fixed syntax error in download() (stray `});` and dead code removed)
 */
...
const icons = {
  ...
  Energy: (p) => <Icon {...p} d="M13 2L3 14h6l-2 8l10-12h-6l2-8z" />, // bolt
  Activity: (p) => <Icon {...p} d="M9 18l-2-3 3-4 2 2 3-5 2 2M5 21h4M15 21h4" />, // runner
  Symptoms: (p) => <Icon {...p} d="M12 2v6M9 5h6M5 12h14M7 12v8h10v-8" />, // clinic cross/bed
  HeartHealth: (p) => (
  ...
  Intimacy: (p) => <Icon {...p} d="M12 21s-7-4.5-7-9a4 4 0 0 1 7-2a4 4 0 0 1 7 2c0 4.5-7 9-7 9z" />, // heart
  Back: (p) => <Icon {...p} d="M15 18l-6-6l6-6" />,
  Dashboard: (p) => <Icon {...p} d="M3 3h8v8H3zM13 3h8v5h-8zM13 10h8v11h-8zM3 13h8v8H3z" />,
  Track: (p) => <Icon {...p} d="M3 6h18M3 12h18M3 18h18" />, // list
  Journal: (p) => <Icon {...p} d="M5 3h10l4 4v14H5zM15 3v4h4" />, // file
  Flags: (p) => <Icon {...p} d="M6 3v18M6 4h10l-2 3l2 3H6" />, // flag
};
...
const toCSV = (rows) => {
  ...
  const header = ["timestamp", "category", "path", "value"].map(esc).join(",");
  const body = rows
    .map((r) =>
      [
        r.timestamp,
...
function download(filename, text) {
  try {
    ...
    a.click();
    // Delay revoke/remove to avoid cancelling the download in Safari/iOS
    setTimeout(() => {
      try {
    ...
    // Fallback: try data URI navigation
 ...
 * Theming helpers
 ****************/
const shellClass = "min-h-screen bg-black text-[#F6E7B6]"; // soft gold text
const containerClass = "max-w-md mx-auto px-4 pb-24 pt-8 text-center"; // bottom space for fixed nav
const panelClass =
  ...
  return (
    <div className="flex gap-2 justify-center">
      <input
        ...
        autoFocus={autoFocus}
      />
      <button className={`${buttonClass} px-3`} onClick={() => v.trim() && onSubmit(v.trim())}>
        Add
      </button>
    </div>
  );
...
function SleepWizard({ onDone, onSave }) {
  ...
  const [data, setData] = useState({
    type: null, duration: null, hadDream: null, dreamType: null, dreamLength: null,
  });
...
function Drilldown({ category, onDone, onSave }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <button className="underline text-xs text-yellow-200/70 rounded-lg px-2 py-1 transition hover:bg-yellow-400/10 hover:shadow-[0_0_12px_rgba(255,215,0,0.35)] hover:text-yellow-200" onClick={onDone}>
          Cancel
        </button>
      </div>
    </div>
  ...
  const renderExpanded = (e) => {
    ...
    if (e.category === "Sleep" && v && typeof v === "object") {
      const rows = [
        ...
        ...(v.hadDream && v.hadDream !== "None"
          ? [
              ["Dream Type", v.dreamType],
              ["Dream Length", v.dreamLength],
            ]
          : []),
      ];
      return (
    ...
    <div className="space-y-4">
      <div className="flex gap-2 justify-center">
        ...
        <button
          ...
          onClick={() =>
            download(
              `track_${new Date().toISOString().slice(0, 10)}.csv`,
              toCSV(filtered),
            )
          }
        >
      ...
      <div className="space-y-2">
        {filtered.length === 0 && (
          <div className="text-sm text-yellow-200/70">No entries yet.</div>
        )}

        {filtered.map((e, i) => {
          const isOpen = expanded.has(e.id);
          const summary =
            e.category === "Sleep" ? sleepSummary(e.value) : genericSummary(e.value);
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 overflow-hidden">
                <div className="text-[11px] text-yellow-200/70 whitespace-nowrap">
                  {new Date(e.timestamp).toLocaleString()}
                </div>
                <div className="text-yellow-200/40">•</div>
                <div className="font-medium whitespace-nowrap">{e.category}</div>
                <div className="text-yellow-200/40">•</div>
                <div className="text-xs text-yellow-100/80 whitespace-nowrap">{e.path}</div>
              </div>

              {/* compact one-liner that never overflows */}
              <div
                className={`mt-1 ${
                  isOpen
...
function Journal({ journal, setJournal }) {
  ...
  const exportCSV = () => {
    ...
    const body = journal
      .map((j) => '"' + j.timestamp + '","' + j.text.replace(/"/g, '""') + '"')
      .join("\n");
...
export default function App() {
  ...
  // Lightweight runtime tests (in dev/preview)
  useEffect(() => {
    try {
      // Test 1: icons map has required keys and functions
      const iconKeys = [
        "Food",
        "Sleep",
        "Mood",
      ...
      iconKeys.forEach((k) => {
        console.assert(typeof icons[k] === "function", `Icon missing or not a function: ${k}`);
      });

      // Test 2: categories have id/icon/nodes
      CATEGORIES.forEach((c) => {
        console.assert(c.id && icons[c.id] && c.icon, `Bad category config: ${c.id}`);
        console.assert(
          Array.isArray(c.nodes) || typeof c.nodes === "object",
          `Category nodes must be array or object: ${c.id}`,
        );
      });

      // Test 3: drillChildren behavior
      ...
      // Test 4: toCSV header/escaping and row count
      const csv = toCSV([{ timestamp: "2025-01-01T00:00:00Z", category: "Test", path: "X", value: 'a"b' }]);
      console.assert(csv.startsWith('"timestamp","category","path","value"'), "toCSV header missing");
      console.assert(csv.split("\n").length === 2, "toCSV should have 2 lines");
      console.assert(csv.includes('"a""b"'), "toCSV should escape quotes");

      // Test 5: HeartHealth → Palpitations has deep options
  ...
  function useToast() {
    ...
    const el = (
      <div
        className={`fixed left-1/2 -translate-x-1/2 bottom-24 px-4 py-2 rounded-2xl shadow-lg text-sm bg-yellow-500/20 text-yellow-200 border border-yellow-400/30 ${
          msg ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
        role="status"
        aria-live="polite"
      >
  ...
  function saveEntry(category, path, value) {
    const e = { id: crypto.randomUUID(), timestamp: nowISO(), category, path, value };
  ...
  return (
    <div className={shellClass}>
      <div className={containerClass}>
        {/* Big Heading */}
        <h1 className="text-3xl font-semibold tracking-wide drop-shadow-[0_0_10px_rgba(255,215,0,0.25)] mb-6">
        ...
        <main className="space-y-6">
          ...
          {tab === "Dashboard" && drilling && activeCategory && (
            <Drilldown
              ...
              onSave={({ value }) => {
                const path = activeCategory.id;
                saveEntry(activeCategory.id, path, value);
              }}
            />
          )}
        ...
        {/* Footer */}
        <footer className="pt-8 text-center text-[11px] text-yellow-200/60">
          ...
          {[
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 py-2 rounded-xl ${
                tab === t.id ? "bg-yellow-400/10 border border-yellow-400/30" : ""
              }`}
              onClick={() => {
                setTab(t.id);
                setDrilling(false);
